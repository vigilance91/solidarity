...
import "https://github.com/vigilance91/solidarity/libraries/bytes32/Bytes32Constraints.sol";
import "https://github.com/vigilance91/solidarity/libraries/bytes32/bytes32ToString.sol";
...
///
/// @title Mixin Hashed Nonces Library
/// @author Tyler R. Drury <vigilstudios.td@gmail.com> (www.twitter.com/StudiosVigil) - copyright 6/12/2021, All Rights Reserved
/// @dev mixin library providing storage and functions for using unique, bytes32 specific nonces
///
library mixinHashedNonces
{
    ...
    struct NonceStorage{
        mapping(bytes32=>Counters.Counter) nonces;
    }
    ...
    function currentNonce(
        ...
        return uint256(
            nonces(slot)[hash].current().hexadecimal().hash()
        );
    }
    //
    ...
    //)internal view returns(
        //bytes32
    //){
        //return nonces(slot)[hash].current().hexadecimal().hash();
    //}
    ...
    {
        //if(_storageNonces(slot).nonces[hash] ==){
            //_storageNonces(slot).nonces[hash] = 0;
            //return;
        //}
        
        _storageNonces(slot).nonces[hash].increment();
    }
    
    function asHexAndSalt(
        bytes32 slot,
        bytes32 hash
    )internal view returns(
        string memory
    ){
        return hash.hexadecimal().concatenate(
            currentNonce(slot, hash).hexadecimal()
        );
    }
